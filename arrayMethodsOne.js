...
/*
  Use the filter method on mixedNumbers to make a new array of 
...
/*
  ...
  Instead of just arrays of numbers, we are going to have an array of 
  objects that we want to use map, filter, and reduce with.
  Use the filter method to return only the monsters that have a CP of 
  over 200.
*/
...
/*
  ...
  the sales tax (given to you as a tax rate, hint: you'll need to do 
  some multiplication). Your answer should be an array of numbers, one 
  total for each order.
*/



const pricesAfterTax = orders.map((origPrice) => {
  let salesTax = origPrice.price * origPrice.tax 
  let total = origPrice.price + salesTax
  return total
})
